## Versionamento de edições

| Data           | Autor                | Descrição                           |Versão|
|----------------|----------------------|-------------------------------------|------|
|13/06/2019      | William elias alves  | adição da introdução e objetivos    | 1.0|
|16/06/2019      | Aline Laureano  | Adição dos resultados   | 1.1|
|17/06/2019      | Luís Cláudio T. Lima  | Adição de mais links para telas das respectivas funcionalidades   | 1.2|
|19/06/2019      | Aline Laureano  | Adição de nova versão da tabela de resultados   | 1.3 |
|19/06/2019      | Guilherme Marques  | Adiciona novos dados na tabela   | 1.4 |

### Introdução

Rastreável é uma característica de sistemas nos quais há uma interligação entre os dados obtidos e as informações geradas. No caso da engenharia de requisitos podemos considerar como rastreabilidade como a caracaterização de relação entre os requisitos e suas fontes ou artefatos criados durante todo o ciclo de vida do software. Essa característa está atrelada intrinsicamente a utilização de uma baseline de requisitos para uma melhor transparência dos requisitos elicitados.

Com a rastreabilidade é possível ver todo o processo evolutivo referente aos requisitos e suas conseguências.
Com isso temos a pré-rastreabilidade como uma projeção de possíveis surgimentos de requisitos e também temos a pós rastreabilidade que será tratada nesse tópico por meio da utilização de algumas técnicas inerentes a ela.

### Objetivo

### Resultados

*Versão 1*

#### Requisitos funcionais

| Responsável | ID | Requisito | Tela da funcionalidade |
|-------------|----|-----------|------------------------|
|  |	R004 |	Pesquisar artigos por nome |  |
| Aline |	R005 |	Visualizar data de publicação dos artigos |	<a href="../telas_das_funcionalidades/#imgR005">Tela(s) do R005</a> |
| Aline |	R010 |	Criar nova 'serie' |	<a href="../telas_das_funcionalidades/#imgR010">Tela(s) do R010</a> |
| Aline |	R012 |	Visualizar 'serie' |	<a href="../telas_das_funcionalidades/#imgR012">Tela(s) do R012</a> |
| Aline |	R013 |	Salvar 'serie' como rascunho | <a href="../telas_das_funcionalidades/#imgR013">Tela(s) do R013</a>|
| Aline |	R014 |	Editar 'serie' | <a href="../telas_das_funcionalidades/#imgR014">Tela(s) do R014</a> |
| Aline |	R015 |	Deletar 'serie' |	 |
| Aline |	R016 |	Ver status |  |
| Aline |	R017 |	Escrever 'new story' |  |
| Aline |	R018 |	Salvar 'story' como rascunho |  |
| Aline |	R019 |	visualizar rascunhos das 'stories' |  |
| Aline |	R020 |	Visualizar stories publicadas |	 |
| Aline |	R021 |	Importar 'story' |  |
| Aline |	R022 |	Editar story |  |
| Aline |	R023 |	Deletar story |	 |
| Aline |	R024 |	Aplaudir um texto lido |  |
| Aline |	R025 |	Compartilhar texto lido nas redes sociais |	 |
| Guilherme Marques |	R026 |	Comentar no corpo do texto da 'story' | <a href="../telas_das_funcionalidades/#imgR026">Tela(s) do R026</a> |		
|  |	R027 |	Escutar podcasts		|  |
|Luís Cláudio|	R028 |	Seguir escritores		|<a href="../telas_das_funcionalidades/#imgR028">Tela(s) do R028</a>  |
||	R029 |	Seguir usuários		|  |
| Guilherme Marques |	R030 |	Receber notificações dos escritores seguidos		| <a href="../telas_das_funcionalidades/#imgR030">Tela(s) do R030</a> |
|  |	R031 |	Selecionar tema de artigos para leitura		|  |
| Aline |	R032 |	Visualizar textos publicados por um escritor em seu perfil		|  |
| Aline |	R033 |	Visualizar os artigos favoritados		|  |
| Aline |	R034 |	Visualizar os artigos curtidos		|  |
| Aline |	R035 |	Visualizar perfis de outros usuários	|  |
| Guilherme Marques |	R036 |	Favoritar publicação para leitura posterior		| <a href="../telas_das_funcionalidades/#imgR036">Tela(s) do R036</a> |
| Guilherme Marques |	R037 |	Visualizar comentários da publicação		| <a href="../telas_das_funcionalidades/#imgR037">Tela(s) do R037</a> |
| Luís Cláudio |	R038 |	Reportar publicação		|<a href="../telas_das_funcionalidades/#imgR038">Tela(s) do R038</a>  |
| Luís Cláudio |	R039 |	Reportar usuário		|<a href="../telas_das_funcionalidades/#imgR039">Tela(s) do R039</a>  |
| Luís Cláudio |	R040 |	Editar informações de perfil		| <a href="../telas_das_funcionalidades/#imgR040">Tela(s) do R040</a> |
| Luís Cláudio |	R041 |	Salvar/cancelar edição de perfil		|<a href="../telas_das_funcionalidades/#imgR041">Tela(s) do R041</a>  |
|  |	R042 |	Visualizar autores seguidos		|  |
|  |	R043 |	Visualizar seguidores		|  |
| Guilherme Marques |	R044 |	Visualizar 'claps' em 'stories'		| <a href="../telas_das_funcionalidades/#imgR044">Tela(s) do R044</a> |
| Luís Cláudio |	R045 |	visualizar próprio perfil		|<a href="../telas_das_funcionalidades/#imgR045">Tela(s) do R045</a>  |
|  |	R046 |	Visualizar estatisticas de leitura em gráfico		|  |
| Guilherme Marques |	R047 |	Visualizar marcações feitas		| <a href="../telas_das_funcionalidades/#imgR047">Tela(s) do R047</a> |
| Luís Cláudio |	R048 |	Criar publicação		| <a href="../telas_das_funcionalidades/#imgR048">Tela(s) do R048</a> |
| Guilherme Marques |	R049 |	visualizar rascunho de publicação		| <a href="../telas_das_funcionalidades/#imgR049">Tela(s) do R049</a> |
| |	R050 |	Editar publicação		|  |
| |	R051 |	Deletar publicação		|  |
| Guilherme Marques |	R052 |	Selecionar tópicos de interesse		| <a href="../telas_das_funcionalidades/#imgR052">Tela(s) do R052</a> |
| Guilherme Marques |	R053 |	Deixar de seguir usuários		| <a href="../telas_das_funcionalidades/#imgR053">Tela(s) do R053</a> |
| Guilherme Marques |	R054 |	Deixar de seguir publicações		| <a href="../telas_das_funcionalidades/#imgR054">Tela(s) do R054</a> |
|    |	R055 |	Ativar/desativar modo noturno		|  |
| Luís Cláudio |	R057 |	Editar email padrão		|<a href="../telas_das_funcionalidades/#imgR057">Tela(s) do R057</a>  |
|  |	R058 |	Selecionar périodo de sugestão de artigos		|  |
|  |	R059 |	Desativar sugestão de artigos		|  |
| Luís Cláudio |	R060 |	Ativar/desativar boletim informativo|<a href="../telas_das_funcionalidades/#imgR060">Tela(s) do R060</a>  |		
|Luís Cláudio  |	R061 |	Selecionar tema do boletim informativo	|<a href="../telas_das_funcionalidades/#imgR061">Tela(s) do R061</a> |
| Luís Cláudio |	R062 |	Ativar/desativar recomendações de stories		|<a href="../telas_das_funcionalidades/#imgR062">Tela(s) do R062</a>  |
| Luís Cláudio |	R063 |	Ativar/desativar notificação por e-mail		|<a href="../telas_das_funcionalidades/#imgR063">Tela(s) do R063</a>  |
| Luís Cláudio |	R064 |	Ativar/desativar notificação social		| <a href="../telas_das_funcionalidades/#imgR064">Tela(s) do R064</a> |
| Luís Cláudio |	R066 |	Desativar menções de usuários		| <a href="../telas_das_funcionalidades/#imgR066">Tela(s) do R066</a> |
| Luís Cláudio |	R067 |	Conectar com redes sociais		|<a href="../telas_das_funcionalidades/#imgR067">Tela(s) do R067</a>  |
| Luís Cláudio |	R068 |	Conectar com Google		|<a href="../telas_das_funcionalidades/#imgR068">Tela(s) do R068</a>  |
| Luís Cláudio |	R069 |	Mostrar links para facebook e twitter na página de perfil		| <a href="../telas_das_funcionalidades/#imgR069">Tela(s) do R069</a> |
|Luís Cláudio  |	R073 |	Fazer download de meus dados do MEDIUM		|<a href="../telas_das_funcionalidades/#imgR073">Tela(s) do R073</a>  |
|  |	R074 |	Deslogar de outras sessões		|  |
| Luís Cláudio |	R075 |	Desativar conta		|<a href="../telas_das_funcionalidades/#imgR075">Tela(s) do R075</a>   |
| Luís Cláudio |	R076 |	Deletar conta		| <a href="../telas_das_funcionalidades/#imgR076">Tela(s) do R076</a> |
|  |	R077 |	Visualizar o tutorial das áreas da aplicação		|  |
|  |	R078 |	Acessar glossário MEDIUM		|  |

***

*Versão 2*

#### Requisitos funcionais

| Tema | Épico | Requisito | Critérios de aceitação | Tela da funcionalidade |
|------|-------|-----------|------------------------|------------------------|
| Serie |  |	R010 -	Criar nova 'serie' | <a href="../../modelagem/sprints_backlog/#caUS059">Link para os critérios</a> |	<a href="../telas_das_funcionalidades/#imgR010">Tela(s) do R010</a> |
| Serie |  |	R012 -	Visualizar 'serie' | <a href="../../modelagem/sprints_backlog/#caUS059">Link para os critérios</a> |	<a href="../telas_das_funcionalidades/#imgR012">Tela(s) do R012</a> |
| Serie |  |	R013 -	Salvar 'serie' como rascunho | <a href="../../modelagem/sprints_backlog/#caUS059">Link para os critérios</a> | <a href="../telas_das_funcionalidades/#imgR013">Tela(s) do R013</a>|
| Serie | Edição |	R014 -	Editar 'serie' |  | <a href="../telas_das_funcionalidades/#imgR014">Tela(s) do R014</a> |
| Serie |  |	R015 -	Deletar 'serie' | <a href="../../modelagem/sprints_backlog/#caUS061">Link para os critérios</a> |	 |
| Perfil |  |	R040 -	Editar informações de perfil		| <a href="../../modelagem/sprints_backlog/#caUS005">Link para os critérios</a>  |<a href="../telas_das_funcionalidades/#imgR040">Tela(s) do R040</a> |
| Perfil |  |	R041 -	Salvar/cancelar edição de perfil		| <a href="../../modelagem/sprints_backlog/#caUS005">Link para os critérios</a> |<a href="../telas_das_funcionalidades/#imgR041">Tela(s) do R041</a>  |
| Perfil |  |	R045 -	visualizar próprio perfil		| <a href="../../modelagem/sprints_backlog/#caUS056">Link para os critérios</a>  |<a href="../telas_das_funcionalidades/#imgR045">Tela(s) do R045</a>  |
| Publicação |  |	R048 -	Criar publicação		|  | <a href="../telas_das_funcionalidades/#imgR048">Tela(s) do R048</a> |
| Story |  |	R005 -	Visualizar data de publicação dos artigos | <a href="../../modelagem/sprints_backlog/#caUS071">Link para os critérios</a> |	<a href="../telas_das_funcionalidades/#imgR005">Tela(s) do R005</a> |
| Story|  |	R016 -	Ver status |  |  |
| Story |  |	R017 -	Escrever 'new story' | <a href="../../modelagem/sprints_backlog/#caUS063">Link para os critérios</a> |  |
| Story |  |	R018 -	Salvar 'story' como rascunho | <a href="../../modelagem/sprints_backlog/#caUS063">Link para os critérios</a> |  |
| Story |  |	R019 -	visualizar rascunhos das 'stories' | <a href="../../modelagem/sprints_backlog/#caUS063">Link para os critérios</a> |  |
| Story |  |	R020 -	Visualizar stories publicadas |	<a href="../../modelagem/sprints_backlog/#caUS063">Link para os critérios</a> |  |
| Story |  |	R021 -	Importar 'story' | <a href="../../modelagem/sprints_backlog/#caUS064">Link para os critérios</a> |  |
| Story | Edição  |	R022 -	Editar story |  |  |
| Story |  |	R023 -	Deletar story |	 |  |
| Story |  |	R024 -	Aplaudir um texto lido | <a href="../../modelagem/sprints_backlog/#caUS071">Link para os critérios</a> |  |
| Story |  |	R025 -	Compartilhar texto lido nas redes sociais |	<a href="../../modelagem/sprints_backlog/#caUS071">Link para os critérios</a> |  |
| Story |  |	R026 -	Comentar no corpo do texto da 'story' | <a href="../../modelagem/sprints_backlog/#caUS071">Link para os critérios</a> |  |
| Story |  |	R044 -	Visualizar 'claps' em 'stories'		| <a href="../../modelagem/sprints_backlog/#caUS071">Link para os critérios</a> |  |
| Story |  |	R049 -	Visualizar rascunho de publicação		|  |  |
| Interação |  |	R028 -	Seguir escritores		| <a href="../../modelagem/sprints_backlog/#caUS028">Link para os critérios</a> | <a href="../telas_das_funcionalidades/#imgR028">Tela(s) do R028</a>  |
| Interação |  |	R030 -	Receber notificações dos escritores seguidos		|  |  |
| Interação |  |	R031 -	Selecionar tema de artigos para leitura		|  |  |
| Interação |  |	R032 -	Visualizar textos publicados por um escritor em seu perfil		|  |  |
| Interação |  |	R033 -	Visualizar os artigos favoritados		|  |  |
| Interação |  |	R034 -	Visualizar os artigos curtidos		|  |  |
| Interação |  |	R035 -	Visualizar perfis de outros usuários	| <a href="../../modelagem/sprints_backlog/#caUS029">Link para os critérios</a> |  |
| Interação |  |	R036 -	Favoritar publicação para leitura posterior		|  |  |
| Interação |  |	R037 -	Visualizar comentários da publicação		| <a href="../../modelagem/sprints_backlog/#caUS031">Link para os critérios</a> |  |
| Interação |  |	R038 -	Reportar publicação		| <a href="../../modelagem/sprints_backlog/#caUS032">Link para os critérios</a> |<a href="../telas_das_funcionalidades/#imgR038">Tela(s) do R038</a>  |
| Interação |  |	R039 -	Reportar usuário		|  |<a href="../telas_das_funcionalidades/#imgR039">Tela(s) do R039</a>  |
| Interação |  |	R042 -	Visualizar autores seguidos		| <a href="../../modelagem/sprints_backlog/#caUS026">Link para os critérios</a> |  |
| Interação |  |	R047 -	Visualizar marcações feitas		|  |  |
| Interação |  |	R053 -	Deixar de seguir usuários		|  |  |
| Interação |  |	R054 -	Deixar de seguir publicações		|  |  |
| Conexões |  |	R067 -	Conectar com redes sociais		| <a href="../../modelagem/sprints_backlog/#caUS006">Link para os critérios</a> |<a href="../telas_das_funcionalidades/#imgR067">Tela(s) do R067</a>  |
| Conexões |  |	R068 -	Conectar com Google		|  |<a href="../telas_das_funcionalidades/#imgR068">Tela(s) do R068</a>  |
|Luís Cláudio  |  |	R073 -	Fazer download de meus dados do MEDIUM		|  |<a href="../telas_das_funcionalidades/#imgR073">Tela(s) do R073</a>  |
| Conta | Conta |	R075 -	Desativar conta		| <a href="../../modelagem/sprints_backlog/#caUS023">Link para os critérios</a> |<a href="../telas_das_funcionalidades/#imgR075">Tela(s) do R075</a>   |
| Conta | Conta |	R076 -	Deletar conta		| <a href="../../modelagem/sprints_backlog/#caUS024">Link para os critérios</a> | <a href="../telas_das_funcionalidades/#imgR076">Tela(s) do R076</a> |
| Configuração |  |	R052 -	Selecionar tópicos de interesse		|  |  |
| Configuração | Edição |	R057 -	Editar email padrão		|  |<a href="../telas_das_funcionalidades/#imgR057">Tela(s) do R057</a>  |
| Configuração |  |	R060 -	Ativar/desativar boletim informativo|  |<a href="../telas_das_funcionalidades/#imgR060">Tela(s) do R060</a>  |		
|Configuração  |  |	R061 -	Selecionar tema do boletim informativo	|  |<a href="../telas_das_funcionalidades/#imgR061">Tela(s) do R061</a> |
| Configuração |  |	R062 -	Ativar/desativar recomendações de stories		|  |<a href="../telas_das_funcionalidades/#imgR062">Tela(s) do R062</a>  |
| Configuração | Notificação |	R063 -	Ativar/desativar notificação por e-mail		| <a href="../../modelagem/sprints_backlog/#caUS019">Link para os critérios</a> |<a href="../telas_das_funcionalidades/#imgR063">Tela(s) do R063</a>  |
| Configuração | Notificação |	R064 -	Ativar/desativar notificação social		| <a href="../../modelagem/sprints_backlog/#caUS010">Link para os critérios</a> | <a href="../telas_das_funcionalidades/#imgR064">Tela(s) do R064</a> |
| Configuração |  |	R066 -	Desativar menções de usuários		|  | <a href="../telas_das_funcionalidades/#imgR066">Tela(s) do R066</a> |
| Configuração |  |	R069 -	Mostrar links para facebook e twitter na página de perfil		|  | <a href="../telas_das_funcionalidades/#imgR069">Tela(s) do R069</a> |
